...
class AskQuestionForm(forms.Form):
    '''提问表单'''
    title = forms.CharField(label='问题标题', widget=forms.TextInput(attrs={'class':'form-control'}), max_length=200, required=True)
    topics = forms.ModelMultipleChoiceField(queryset=Topic.objects.all(), to_field_name='name', label='添加话题', required=True, \
